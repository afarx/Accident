...
# 重编码-----------------------------------------------------------------
edc208 <- edc20[,c('报卡类型','性别编码','证件号码','出生日期','死亡日期','直接死亡原因编码b',
                   '其他疾病诊断代码1','其他疾病诊断代码2','其他疾病诊断代码3',
                   '根本死因代码','户籍区分','死者生前病史及症状体征','尸表（尸解）检验记录')]
...
# 抑郁症（F32.9）自杀筛查------------------------------------------------
dep1 <-  edc208hu %>% filter(ICD.3 %in% 'F32') #未特指的抑郁症 224例
dep1zj <-  dep1 %>% filter(zjICD.3 %in% paste0('X',60:84)) #直接死因自害 160例

## 检查死因描述中有“自杀”、“跳”、“自缢”的说法
...
# 自杀抑郁症筛查-------------------------------------------------------
sui1 <-  edc208hu %>% filter(ICD.3 %in% paste0('X',60:84)) #自杀
#间接死因抑郁
